...
export const ProductTableByCategory = ({ products, onEdit, onDelete }: ProductTableByCategoryProps) => {
  ...
  const groupedProducts = products.reduce((acc, product) => {
    ...
    const subCategoryKey = product.subCategory || "Sem Subcategoria"; // Use a chave "Sem Subcategoria" se não houver
    if (!acc[product.category][subCategoryKey]) {
      acc[product.category][subCategoryKey] = [];
    }
    acc[product.category][subCategoryKey].push(product);
    return acc;
  }, {} as Record<string, Record<string, Product[]>>);

  // Define the custom order for main categories
  const customCategoryOrder = [
    "DLC NEGATIVA PÃES",
    "DLC NEGATIVA PROTEÍNAS",
    "DLC NEGATIVA Outros",
    "DLC Positiva", // Adicionando a nova categoria principal
    "McCafé",
    "Queijos",
    "Molhos",
    "Sobremesas",
  ...
  // Sort main categories based on the custom order, placing unknown categories at the end
  const mainCategories = Object.keys(groupedProducts).sort((a, b) => {
